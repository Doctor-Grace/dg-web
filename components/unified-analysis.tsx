...
import { examTypeToDestination, examTypeToConsumption } from "@/domain/exam.domain"
...
export function UnifiedAnalysis() {
  ...
  const [reasonForRequestingExam, setReasonForRequestingExam] = useState<string>("")
  ...
  return (
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          <label className="text-sm font-medium">Carregar imagens (máximo 10)</label>
          <div
            className={cn(
              "border-2 border-dashed rounded-lg p-6 text-center transition-colors",
              isDragging ? "border-primary bg-primary/5" : "border-muted-foreground/20 hover:border-primary/50",
              !isVerified || isLoading ? "opacity-50 cursor-not-allowed" : "cursor-pointer",
              "relative",
            )}
            onDragOver={handleDragOver}
            onDragLeave={handleDragLeave}
            onDrop={handleDrop}
            onClick={() => (!isVerified || isLoading ? checkAccess() : document.getElementById("file-upload")?.click())}
          >
            <input
              id="file-upload"
              type="file"
            ...
            <div className="flex flex-col items-center justify-center gap-2">
              ...
              <p className="text-sm text-muted-foreground">
                {t("drag_drop_or")} <span className="text-primary font-medium">{t("browse")}</span>
              </p>
        ...
        {files.length > 0 && (
          <div className="space-y-2">
            <label className="text-sm font-medium">Imagens selecionadas ({files.length})</label>
      ...
      <CardFooter>
        <Button className="w-full" onClick={handleSubmit} disabled={isLoading || isChecking || !isVerified}>
          {isLoading ? (
            <>
